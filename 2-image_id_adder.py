...
try:
    ...
    # 5. Chercher la colonne "B_ID"
    id_column = None
    for col in worksheet.iter_cols(min_row=1, max_row=1):
        for cell in col:
            if cell.value == "B_ID":
                id_column = cell.column
                print(f"✅ Colonne 'B_ID' trouvée en position : {openpyxl.utils.get_column_letter(id_column)}")
                break
        if id_column:
            break

    if id_column:
        # 6. Insérer une nouvelle colonne "image-id" juste après "B_ID"
        ...
        # 7. Parcourir les données de la colonne "B_ID" et générer le contenu de la colonne "image-id"
        rows_processed = 0
        for row in worksheet.iter_rows(min_row=2, max_row=worksheet.max_row):
            cell = row[id_column - 1]  # Colonne B_ID
        ...
        print("❌ Erreur : La colonne 'B_ID' n'a pas été trouvée dans la feuille.")
