...
par dossier. L'application de diagnostic actuelle refuserait d'ailleurs le
fichier tel quel. Plusieurs règles métier doivent d'abord être confirmées.

## Faits vérifiés

Dans l'onglet « BDD TT + ANALYSE DGDDI » :

- l'en-tête se trouve ligne 40 et la colonne A contient l'identifiant dossier ;
- 563 lignes avec un identifiant sont visibles et 473 sont masquées, soit
  1 036 lignes au total ;
- 1 034 identifiants sont numériques et se retrouvent exactement dans
  l'onglet « Etablissements » ;
...
- le filtre « Champ = Dossier » donne une ligne par dossier ;
- 1 022 dossiers ont également une ligne « Champ = SIRET », qui désigne le
  même établissement dans la grande majorité des cas ;
- pour 118 dossiers, les lignes « Dossier » et « SIRET » diffèrent : dans
  99 cas le SIRET est identique mais d'autres informations varient, dans 19 cas
  le SIRET lui-même diffère.
...
1. Par défaut, le périmètre est l'union des identifiants dossier non vides,
   soit 1 036 dossiers (1 034 numériques + 2 textuels, lignes masquées incluses).
   Faut-il confirmer ce périmètre ou en exclure explicitement certains
   (identifiants textuels, lignes masquées) ?
...
5. Si « Avis » ou « Composition du produit » sont retenus, comment leurs
   différentes lignes doivent-elles être regroupées en une seule ligne par
   dossier ?
6. « OFG » doit-il être exclu ou faire l'objet d'une règle complémentaire pour
   les SIRET rattachés à plusieurs dossiers ?
...
- aucune ligne ou colonne masquée contenant des données à traiter ;
