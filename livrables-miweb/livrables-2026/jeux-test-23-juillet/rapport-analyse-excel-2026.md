...
- Objectif : contre-analyser le fichier de test 2026, vérifier les chiffres de `mail.md`, confronter la structure aux règles métier 2026 verrouillées (cf. `AGENTS.md`).
- Méthode : lecture indépendante du classeur via `openpyxl`, en lecture seule, sans modifier le fichier source. Annexe pour la reproductibilité.
- Confrontation : ce rapport a été recroisé avec la contre-analyse Codex (`rapport-analyse-excel-2026-codex.md`), qui a en outre exécuté le diagnostic réel. Les chiffres convergent ; les corrections issues de la confrontation sont intégrées et signalées.

---

## 1. Synthèse exécutive

1. **Les chiffres du mail métier sont inexacts sur un point central.** BDD contient **1 036 lignes** à identifiant, dont **1 034 à identifiant numérique** et **2 à identifiant textuel atypique** (`DOSSIER PAPIER`, `SARL VERRESATINE`). Le mail confond 1 034 et 1 036 et laisse entendre une incohérence entre BDD et Etablissements : en réalité les deux onglets sont **alignés à zéro écart sur les 1 034 identifiants numériques**.
...
4. **Plusieurs décisions métier restent à valider** : statut des 2 identifiants atypiques, sort des 473 lignes masquées de BDD, choix de l'objet dans Etablissements (118 dossiers ont deux lignes qui diffèrent), périmètre des onglets à faible couverture (Avis : 8 ; Composition : 783 sur 1 034).
...
- 1 034 identifiants sont numériques ; 2 sont textuels et atypiques :
  - ligne 224 : `ID = "DOSSIER PAPIER"` — données partielles (Bourgogne-Franche-Comté, Nièvre, « Transformateur de courant ») ;
  - ligne 1 076 : `ID = "SARL VERRESATINE"` — ligne quasi vide.
- **Prudence** : le caractère numérique ne suffit pas à qualifier les 1 034 de « vrais dossiers », ni les 2 autres d'invalides. Leur statut métier (test, anomalie, dossier papier légitime) reste à confirmer. *(Correction issue de la confrontation Codex.)*
...
- Les 473 identifiants masqués de BDD sont tous présents dans Etablissements avec SIRET et adresse non vides. Cela **prouve leur présence** dans le référentiel, **pas leur éligibilité** au publipostage — leur statut (actif, rejeté, archivé) reste à confirmer. *(Nuance issue de la confrontation Codex.)*
...
- 53 lignes ont un SIRET, 1 n'en a pas (« LEMEILLEUR CASSANDRE ») ; 50 SIRET uniques.
- Tous les SIRET sont présents dans Etablissements (`Champ = SIRET`), **aucun orphelin**. **Mais le rattachement n'est pas univoque** : 4 SIRET (couvrant 7 lignes OFG) renvoient vers **plusieurs dossiers** (ex. `51190070600033` → 3 dossiers). OFG ne se rattache donc pas à un dossier unique sans règle métier. *(Correction issue de la confrontation Codex ; ma version initiale disait à tort « 100 % rattachable ».)*
...
Confrontation aux critères de la règle « refuser l'import si structure ambiguë » (`AGENTS.md` et contrat `docs/specs/2026-07-21-...`) :
...
**Exécution réelle du diagnostic (point bloquant).** `diagnose_workbook` retourne `importable = false` avec **huit blocages** *(constat établi par la contre-analyse Codex, qui a exécuté le code)* :
...
Une version corrigée du mail est proposée dans `mail-correction-proposee.md` (le fichier `mail.md` n'a pas été modifié). La contre-analyse Codex propose `mail-correction-proposee-codex.md`, également utilisable.

---

## 6. Décisions métier à valider

1. **Périmètre final** : 561 identifiants numériques visibles, 1 034 incluant les lignes masquées, ou un autre périmètre ?
2. **Deux identifiants textuels** (`DOSSIER PAPIER`, `SARL VERRESATINE`) : les exclure, les signaler, ou les traiter manuellement ? *(La règle verrouillée ne les couvre pas : elle vise les identifiants vides, et ceux-ci sont non vides.)*
...
4. **Onglet maître** : BDD ou Etablissements comme référence de la liste finale ? (Les deux coïncident.)
5. **Onglet Etablissements** : la colonne `Champ` indique le **contexte source** de la ligne ; le statut « siège social » est une **dimension indépendante** (col E). Sur les 1 022 dossiers ayant une ligne `Dossier` et une ligne `SIRET`, les deux lignes désignent le **même établissement dans 1 003 cas** (mais pas systématiquement : 19 SIRET différents). Les 118 différences sont donc des écarts de remplissage entre deux contextes de saisie du même établissement, pas un choix entre deux objets. Quand les deux lignes diffèrent, quelle ligne et quelles colonnes faut-il retenir (éventuellement colonne par colonne) ?
6. **Onglets multi-lignes (Avis, Composition)** : intégrer (cellules vides + signalisation) ou exclure ? Règle d'agrégation quand un dossier a plusieurs lignes ?
7. **OFG** : exclure, ou définir une règle de rattachement sachant que 4 SIRET pointent vers plusieurs dossiers ?
8. **Gabarit InDesign 2026** (point ouvert `TODO.md:65`) : nouveaux noms `id_dossier`/mapping, ou réemploi des champs 2025 ?
9. **Fichier nettoyé ou application tolérante** : demander un classeur nettoyé (onglets utiles, une ligne par dossier), ou permettre à l'application d'écarter/transformer certains onglets avant le diagnostic ?

---

## 7. Annexe — méthode reproductible

Analyse avec `openpyxl` (lecture seule, `data_only=True` puis `False` pour valeurs/formules), via `uv run --with openpyxl python <script>`.
...
- Conflits Dossier/SIRET : pour les 1 022 dossiers ayant les deux lignes, comparaison colonne par colonne (hors `Dossier ID` et `Champ`) → 118 cas de différence.
...
- L'exécution réelle de `diagnose_workbook` (huit blocages) a été réalisée par la contre-analyse Codex, pas dans ce rapport. Les conclusions convergent.
