...
# de traiter automatiquement le fichier "05-dates-formattees.xlsx",
# de créer une copie finale sous le nom "06-livrable-final.xlsx",
...
from sircom2026_rules import config_value

# 1. Définir le fichier source
file_path = config_value("step_05_output")

# 2. Vérifier que le fichier source existe
if not os.path.exists(file_path):
    print(f"Erreur : Le fichier '{file_path}' n'existe pas dans le répertoire courant.")
    print(
        "Assurez-vous d'avoir exécuté le script '5-changer-date-format.py' au préalable."
    )
    exit(1)
...
try:
    ...
    output_file_name = config_value("step_06_output")
    ...
    print(
        f"  - Données préservées : {worksheet_copy.max_row} lignes, {worksheet_copy.max_column} colonnes"
    )
    ...
    print(
        f"Erreur : Permission refusée. Vérifiez que le fichier '{output_file_name}' n'est pas ouvert dans Excel."
    )
    exit(1)
