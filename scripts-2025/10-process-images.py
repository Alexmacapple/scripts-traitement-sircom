...
def find_best_match(target_name, available_files, logger):
    ...
    exact_candidates = [
        filename for filename in available_files if filename == target_name
    ]
...
def main():
    ...
    try:
        ...
        for dossier_id, image_mapping in mapping.items():
            ...
            if matched_file:
                ...
                success, file_size = process_and_rename_image(
                    source_path, new_name, target_dir, logger
                )
                if success:
                    success_count += 1
        ...
        unused_images = [
            img for img in available_images.keys() if img not in used_images
        ]
        if unused_images:
        ...
        logger.info(
            f"Espace disque utilisé : {total_size:,} octets ({total_size / 1024 / 1024:.2f} MB)"
        )
