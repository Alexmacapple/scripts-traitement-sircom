...
def verify_data_integrity():
    ...
    try:
        ...
        df_excel = pd.read_excel(
            excel_file, dtype=str, na_values=None, keep_default_na=False
        )
        ...
        df_csv = pd.read_csv(
            csv_file, encoding="utf-16", dtype=str, keep_default_na=False
        )
        ...
        col_id_excel = find_column(
            df_excel.columns,
            ["f_id", "id dossier", "id_dossier", "ID du dossier", "B_ID"],
        )
        col_produit_excel = optional_column(
            df_excel.columns, ["produit", "dénomination", "nom produit"]
        )
        col_entreprise_excel = optional_column(
            df_excel.columns, ["entreprise", "nom entreprise"]
        )
        col_image_excel = optional_column(
            df_excel.columns, ["photo", "image", "photo produit"]
        )
        ...
        for idx, row in df_excel.iterrows():
            ...
            if id_str not in ["#N/A", ""]:
                ...
                excel_data[id_str] = {
                    "entreprise": str(row[col_entreprise_excel])
                    if col_entreprise_excel != "#N/A"
                    else "#N/A",
                    "produit": str(row[col_produit_excel])
                    if col_produit_excel != "#N/A"
                    else "#N/A",
                    "image_source": str(row[col_image_excel])
                    if col_image_excel != "#N/A"
                    else "#N/A",
        ...
        for idx, row in df_csv.iterrows():
            ...
            csv_entreprise = (
                str(row[col_entreprise_csv]) if col_entreprise_csv != "#N/A" else "#N/A"
            )
            csv_produit = (
                str(row[col_produit_csv]) if col_produit_csv != "#N/A" else "#N/A"
            )
        ...
        for _idx, row in df_csv.iterrows():
            ...
            csv_image = str(row[col_image_csv])
            expected_image = (
                f"dossier-{csv_id.lower().replace('.', '').replace(' ', '')}.jpg"
            )
            if csv_image == expected_image:
                image_ok_count += 1
